...
function builtinSayAction(pname) {
    ...
    if (pname instanceof Ast.Value) {
        ...
        return new Ast.Action.Invocation(Ast.Invocation(selector, 'say', [param], _loader.standardSchemas.say),
            _loader.standardSchemas.say.removeArgument('message'));
    } else if (pname) {
        let param = new Ast.InputParam('message', new Ast.Value.VarRef(pname));
        return new Ast.Action.Invocation(new Ast.Invocation(selector, 'say', [param], _loader.standardSchemas.say),
            _loader.standardSchemas.say.removeArgument('message'));
    } else {
        return new Ast.Action.Invocation(new Ast.Invocation(selector, 'say', [], _loader.standardSchemas.say),
            _loader.standardSchemas.say.removeArgument('message'));
    }
}
